				...
				const struct pci_device_id *id)
{
	...
	/* DMA Buffer */
	bpdev->dma_buffer = (void *)__get_free_page(GFP_KERNEL);
	if (!bpdev->dma_buffer) {
		printk("System Error: DMA Memory Lake.\n");
		r = -ENOMEM;
		goto err_mm;
	}
	bpdev->dma_addr = __pa(bpdev->dma_buffer);

	/* set DDR nocache */
	r = set_memory_uc((unsigned long)bpdev->dma_buffer, 1);
	if (r < 0) {
		printk("System Error: Set Memory UC failed.\n");
		goto err_uc;
	}
...
{
	/* Set DDR WB */
	set_memory_wb((unsigned long)bpdev->dma_buffer, 1);
	free_page((unsigned long)bpdev->dma_buffer);
	Broiler_pci_uninit(pdev);
}
...
MODULE_DESCRIPTION("NoCACHE DMA");
