...
{
	/* TODO */
	printk("Broiler Asynchronous with IRQ %d\n", irq);
	return IRQ_HANDLED;
}
...
static int __init BiscuitOS_init(void)
{
	...
	r = request_irq(irq, Broiler_irq_handler,
				IRQF_TRIGGER_HIGH,
				"Broiler-PIO-vPIC", NULL);
	if (r < 0) {
		printk("ERROR: Request IRQ failed.\n");
		goto err_irq;
	}

	printk("Asynchronous I/O Port register success.\n");

	/* Trigger Interrupt */
	iowrite16(0x01, iomem + DOORBALL_REG);
...
MODULE_DESCRIPTION("Broiler Asynchonous PIO");
