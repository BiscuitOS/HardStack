...
/*
 * PageFault with Userfaultfd on Fork
 *
...
static void *fault_handler_thread(void *arg)
{
	...
	for (;;) {
		...
		/* Expect only EVENT FORK */
		if (msg.event == UFFD_EVENT_FORK)
			printf("Trigger FORK from UFFD.\n");
	}
}
...
int main()
{
	...
	if ((uffdio_api.features & UFFD_FEATURE_EVENT_FORK) == 0)
		errExit("Don't Support UFFD_FEATURE_EVENT_FORK");
	...
	/* Read Ops, Trigger #PF but don't UFFD */
	ch = *base;
	printf("UFFD %#lx => %c\n", (unsigned long)base, ch);

	/* Fork Ops, Trigger UFFD */
	fork(); 
