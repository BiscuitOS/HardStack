...
/*
 * PageFault with Userfaultfd on SHMEM
 *
...
int main()
{
	...
	if ((uffdio_api.features & UFFD_FEATURE_MISSING_SHMEM) == 0)
		errExit("Don't Support UFFD_FEATURE_MISSING_SHMEM");
	...
	/* Read Ops, Trigger #PF and UFFD-SHMEM */
	ch = *base;
	printf("UFFD SHMEM %#lx => %c\n", (unsigned long)base, ch);
