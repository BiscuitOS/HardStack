/*
 * MEMBLOCK Memory Allocator: memblock_alloc_from
 *
...
#define MEMBLOCK_FAKE_FROM	0x1000000
...
int __init BiscuitOS_Running(void)
{
	...
	/* Alloc Memory */
	mem = memblock_alloc_from(MEMBLOCK_FAKE_SIZE,
				SMP_CACHE_BYTES, MEMBLOCK_FAKE_FROM);
	if (!mem) {
