...
{
	printf("BiscuitOS: sys_umask helper\n");
	printf("Usage:\n");
	...
	printf("\t\t\tS_IXOTH\n");
	printf("\t\t\tS_IRWXO\n");
	printf("\t-u\t--mask\tThe mode for masking.\n");
	printf("\t\t\tS_IRUSR\n");
	printf("\t\t\tS_IWUSR\n");
	printf("\t\t\tS_IXUSR\n");
	printf("\t\t\tS_IRWXU\n");
...
int main(int argc, char *argv[])
{
	...
	/* options */
	const char *short_opts = "hp:f:m:u:";
	...
	if (hflags || !path || !flags || !mode || !umask) {
		usage(argv[0]);
		return 0;
	}
	...
	/* parse umask argument */
	if (umask) {
		if (strstr(umask, "S_IRUSR"))
			omask |= S_IRUSR;
		if (strstr(umask, "S_IWUSR"))
			omask |= S_IWUSR;
		if (strstr(umask, "S_IXUSR"))
			omask |= S_IXUSR;
		if (strstr(umask, "S_IRWXU"))
			omask |= S_IRWXU;
		if (strstr(umask, "S_IRGRP"))
			omask |= S_IRGRP;
		if (strstr(umask, "S_IWGRP"))
			omask |= S_IWGRP;
		if (strstr(umask, "S_IXGRP"))
			omask |= S_IXGRP;
		if (strstr(umask, "S_IRWXG"))
			omask |= S_IRWXG;
		if (strstr(umask, "S_IROTH"))
			omask |= S_IROTH;
		if (strstr(umask, "S_IWOTH"))
			omask |= S_IWOTH;
		if (strstr(umask, "S_IXOTH"))
			omask |= S_IXOTH;
		if (strstr(umask, "S_IRWXO"))
			omask |= S_IRWXO;
	}
