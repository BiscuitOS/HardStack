...
static void usage(const char *program_name)
{
	...
	printf("Usage:\n");
	printf("      %s <-l length> <-f flags> <-m mode>\n", program_name);
	printf("\n");
	printf("\t-l\t--length\tThe path length.\n");
...
int main(int argc, char *argv[])
{
	...
	if (strstr(flags, "O_PATH"))
		oflags |= O_PATH;
	if (strstr(flags, "O_NDELAY"))
		oflags |= O_NDELAY;
	if (strstr(flags, "FASYNC"))
		oflags |= FASYNC;
