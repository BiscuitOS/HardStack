...
{
	printf("BiscuitOS: sys_ftruncate64 helper\n");
	printf("Usage:\n");
...
int main(int argc, char *argv[])
{
	...
	if (hflags || !path || !flags || !mode || (size < 0)) {
		usage(argv[0]);
		return 0;
	}
	 ...
	 * sys_ftruncate64
	 *
	 *    YSCALL_DEFINE2(ftruncate64,
	 *                   unsigned int, fd,
	 *                   loff_t, length)
	 */
	syscall(__NR_ftruncate64, fd, (unsigned long)size);
