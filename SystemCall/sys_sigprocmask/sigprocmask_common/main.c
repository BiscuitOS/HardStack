...
{
	printf("BiscuitOS: sys_sigprocmask helper\n");
	printf("Usage:\n");
	printf("      %s <-s signal>\n", program_name);
	printf("\n");
...
int main(int argc, char *argv[])
{
	...
	act.sa_flags = 0;
	/* prepare capture special signal */
	sigaction(signo, &act, 0);
	sigemptyset(&new);
	sigaddset(&new, signo);

	/* suspend special signal */
	/*
	 * sys_sigprocmask
	 *
	 ...
	 */
	syscall(__NR_sigprocmask, SIG_BLOCK, &new, &old);
	printf("Blocked.\n");
	/* cansole suspend */
	syscall(__NR_sigprocmask, SIG_SETMASK, &old, NULL);
	/* pause: stop until capture speical signal */
	pause();
