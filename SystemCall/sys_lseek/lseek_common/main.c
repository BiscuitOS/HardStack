...
{
	printf("BiscuitOS: sys_lseek helper\n");
	printf("Usage:\n");
	...
	printf("\t\t\tS_IXOTH\n");
	printf("\t\t\tS_IRWXO\n");
	printf("\t-o\t--offset\tThe offset for seeking.\n");
	...
	printf("%s -p BiscuitOS_file -f O_RDWR,O_CREAT "
		  "-m S_IRUSR,S_IRGRP -o 0x20 -w SEEK_SET\n\n", 
							program_name);
}
...
int main(int argc, char *argv[])
{
	...
	/* options */
	const char *short_opts = "hp:f:m:o:w:";
	 ...
	 *                    unsigned int, whence)
	 */
	syscall(__NR_lseek, fd, offset, where);
