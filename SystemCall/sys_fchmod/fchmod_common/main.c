...
{
	printf("BiscuitOS: sys_fchmod helper\n");
	printf("Usage:\n");
	printf("      %s <-p pathname> <-f flags> <-m open_mode> "
			"<-M change_mode>\n", program_name);
	printf("\n");
	...
	printf("\t\t\tO_SYNC\n");
	printf("\t\t\tO_PATH\n");
	printf("\t-m\t--open_mode\tThe mode for opening.\n");
	printf("\t\t\tS_IRUSR\n");
	printf("\t\t\tS_IWUSR\n");
	printf("\t\t\tS_IXUSR\n");
	printf("\t\t\tS_IRWXU\n");
	...
	printf("\t\t\tS_IXOTH\n");
	printf("\t\t\tS_IRWXO\n");
	printf("\t-M\t--chage_mode\tThe mode for changing.\n");
	printf("\t\t\tS_IRUSR\n");
	printf("\t\t\tS_IWUSR\n");
	printf("\t\t\tS_IXUSR\n");
	printf("\t\t\tS_IRWXU\n");
...
int main(int argc, char *argv[])
{
	...
	/* options */
	const char *short_opts = "hp:f:m:M:";
	const struct option long_opts[] = {
		{ "help", no_argument, NULL, 'h'},
		{ "path", required_argument, NULL, 'p'},
		{ "flags", required_argument, NULL, 'f'},
		{ "open_mode", required_argument, NULL, 'm'},
		{ "chag_mode", required_argument, NULL, 'M'},
		{ 0, 0, 0, 0 }
	};
	...
	if (hflags || !path || !flags || !mode || !chgmode) {
		usage(argv[0]);
		return 0;
	}
	...
	/* parse change mode argument */
	if (strstr(chgmode, "S_IRUSR"))
		cmode |= S_IRUSR;
	if (strstr(chgmode, "S_IWUSR"))
		cmode |= S_IWUSR;
	if (strstr(chgmode, "S_IXUSR"))
		cmode |= S_IXUSR;
	if (strstr(chgmode, "S_IRWXU"))
		cmode |= S_IRWXU;
	if (strstr(chgmode, "S_IRGRP"))
		cmode |= S_IRGRP;
	if (strstr(chgmode, "S_IWGRP"))
		cmode |= S_IWGRP;
	if (strstr(chgmode, "S_IXGRP"))
		cmode |= S_IXGRP;
	if (strstr(chgmode, "S_IRWXG"))
		cmode |= S_IRWXG;
	if (strstr(chgmode, "S_IROTH"))
		cmode |= S_IROTH;
	if (strstr(chgmode, "S_IWOTH"))
		cmode |= S_IWOTH;
	if (strstr(chgmode, "S_IXOTH"))
		cmode |= S_IXOTH;
	if (strstr(chgmode, "S_IRWXO"))
		cmode |= S_IRWXO;
