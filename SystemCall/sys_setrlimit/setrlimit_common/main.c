...
{
	printf("BiscuitOS: sys_setrlimit helper\n");
	printf("Usage:\n");
	printf("      %s <-l resource_item> <-c current_limit> "
			"<-m max_limit>\n", program_name);
	printf("\n");
	...
	printf("\t\t\t\n");
	printf("\t-c\t--cur\tThe current resource limit.\n");
	printf("\t-m\t--max\tThe maximum resource limit.\n");
	printf("\ne.g:\n");
	printf("%s -l RLIMIT_FSIZE -c 100 -m 200\n\n", program_name);
}
...
int main(int argc, char *argv[])
{
	char *res = NULL;
	int c, hflags = 0;
	struct rlimit limit, glimit;
	unsigned int item;
	
	opterr = 0;

	/* options */
	const char *short_opts = "hl:c:m:";
	const struct option long_opts[] = {
		{ "help", no_argument, NULL, 'h'},
		{ "res", required_argument, NULL, 'l'},
		{ "cur", required_argument, NULL, 'c'},
	...
	while ((c = getopt_long(argc, argv, short_opts, 
		...
		case 'l': /* limit resource */
			res = optarg;
			break;
		case 'c': /* current value */
			sscanf(optarg, "%ld", &limit.rlim_cur);
			break;
		case 'm': /* maximum value */
			sscanf(optarg, "%ld", &limit.rlim_max);
			break;
		default:
			abort();
		}
	 ...
	 */
	syscall(__NR_setrlimit, item, &limit);
