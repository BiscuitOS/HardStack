...
{
	printf("BiscuitOS: sys_read helper\n");
	printf("Usage:\n");
	printf("      %s <-p pathname> <-f flags> <-m mode> <-s size> "
					"<-l offset>\n", program_name);
	printf("\n");
	...
	printf("\t\t\tS_IXOTH\n");
	printf("\t\t\tS_IRWXO\n");
	printf("\t-s\t--size    The number of bytes for reading.\n");
	printf("\t-l\t--offset  The start read offset for reading.\n");
	printf("\ne.g:\n");
	printf("%s -p BiscuitOS_file -f O_RDONLY,O_NONBLOCK "
			"-m S_IRUSR,S_IRGRP -s 15 -l 0\n\n", program_name);
}
...
int main(int argc, char *argv[])
{
	...
	/* options */
	const char *short_opts = "hp:f:m:s:l:";
	const struct option long_opts[] = {
		...
		{ "offset", required_argument, NULL, 'l'},
		{ 0, 0, 0, 0 }
	};
	...
	sscanf(size, "%d", &esize);
	sscanf(offset, "%ld",  &eoffset);

	/* Establish buffer to store data */
	buffer = (char *)malloc(esize + 8);
	if (!buffer) {
		printf("malloc: no free memory\n");
		return -1;
	}
	 ...
	 *                    size_t, count) 
	 */
	len = syscall(__NR_read, fd, buffer, esize);
	if (len < 0) {
		printf("sys_read: Failed to read\n");
	} else {
		buffer[esize] = '\0';
