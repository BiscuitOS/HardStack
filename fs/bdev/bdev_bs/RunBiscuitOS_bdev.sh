...
mount_fs()
{
	insmod /lib/modules/$(uname -r)/extra/bdev.ko
	lsmod
	[ -d /bdev_bs ] && rm -rf /bdev_bs
	mkdir -p /bdev_bs
	mount_common-0.0.1 -n BiscuitOS_bdev -d /bdev_bs -t bdev_bs -f MS_SILENT -o size=8M
	cd /bdev_bs
