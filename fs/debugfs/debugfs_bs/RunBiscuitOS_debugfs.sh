...
mount_fs()
{
	insmod /lib/modules/$(uname -r)/extra/debugfs.ko
	lsmod
	[ -d /debugfs_bs ] && rm -rf /debugfs_bs
	mkdir -p /debugfs_bs
	mount_common-0.0.1 -n BiscuitOS_debugfs -d /debugfs_bs -t debugfs_bs -f MS_SILENT -o size=8M
	cd /debugfs_bs
