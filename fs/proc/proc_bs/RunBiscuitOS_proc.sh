...
mount_fs()
{
	insmod /lib/modules/$(uname -r)/extra/proc.ko
	lsmod
	[ -d /proc_bs ] && rm -rf /proc_bs
	mkdir -p /proc_bs
	mount_common-0.0.1 -n BiscuitOS_proc -d /proc_bs -t proc_bs -f MS_SILENT -o size=8M
	cd /proc_bs
