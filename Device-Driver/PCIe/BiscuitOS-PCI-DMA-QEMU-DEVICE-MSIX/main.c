...
#define DMA_FW_LEN			0x2000 /* 2 Pages */
...
static void BiscuitOS_pci_realize(PCIDevice *pdev, Error **errp)
{
	...
	/* PCI MMIO BAR */
	memory_region_init_io(&bps->bar_mmio, OBJECT(bps), &BiscuitOS_mmio_ops, bps,
				"BiscuitOS-BAR", MSIX_SIZE);
	...
	pci_register_bar(pdev, BAR_MMIO,
				PCI_BASE_ADDRESS_SPACE_MEMORY, &bps->bar_mmio);
