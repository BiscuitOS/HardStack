 ...
 *
 */
static int pcf8574a_read(struct i2c_client *client, 
 ...
 *
 */
static int pcf8574a_write(struct i2c_client *client, unsigned char offset,
							unsigned char data)
{
	struct i2c_msg msgs;
	unsigned char tmp[2];
				...
				const struct i2c_device_id *id)
{
	...
	ret = request_irq(priv->irq, pcf8574_isr,
				IRQF_TRIGGER_FALLING, DEV_NAME, client);
	if (ret < 0) {
	...
	/* Pull up GPIO4 and GPIO8, other Pull down */
	pcf8574a_write(client, GPIO_ALL, GPIO4 | GPIO8);
