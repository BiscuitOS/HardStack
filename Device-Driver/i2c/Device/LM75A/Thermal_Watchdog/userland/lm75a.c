...
/* OS mode */
#define OS_ACTV_HIGH		0x04
#define OS_ACTV_LOW		0x00
#define OS_CMP_MODE		0x00
#define OS_INTR_MODE		0x02
#define NORMAL_MODE		0x00
#define SHUTDW_MODE		0x01
...
/* Temp register to °C */
static float tempr_to_C(unsigned char *buf)
{
	int value;
	
	value = ((buf[0] & TEMP_MSB_MASK) << 3) | 
					((buf[1] >> 5) & TEMP_LSB_MASK);
	return (float)value * 0.125;
}

/* Tos/Thyst to °C */
static float OS_to_C(unsigned char *buf)
{
	int value;

	value = ((buf[0] & TEMP_MSB_MASK) << 1) | ((buf[1] >> 7) & 0x1);
	return (float)value * 0.5;
}

/* °C to Tos/Thyst */
static void C_to_OS(float temp, unsigned char *buf)
{
	float value;
	int tmp;
	...
	value = temp / 0.5;
	tmp = value;
	buf[0] = (tmp >> 1) & TEMP_MSB_MASK;
	buf[1] = (tmp & 0x1) & TEMP_MSB_MASK;
}
...
int main()
{
	...
	/* Setup LM75A mode: OS active LOW, OS compare mode */
	lm75a_write(fd, I2C_ADDR, LM75A_CONFIG_REG,
			NORMAL_MODE | OS_CMP_MODE | OS_ACTV_LOW);
	/* Setup: Tos --> 50°C Thyst --> 40°C */
	memset(buf, 0, 2);
	C_to_OS(50.0, buf);
	lm75a_2bytes_write(fd, I2C_ADDR, LM75A_TOS_REG, buf);
	memset(buf, 0, 2);
	C_to_OS(40.0, buf);
	lm75a_2bytes_write(fd, I2C_ADDR, LM75A_THYST_REG, buf);
	/* Confirm */
	memset(buf, 0, 2);
	lm75a_read(fd, I2C_ADDR, LM75A_CONFIG_REG, buf);
	printf("Configure: %hhx\n", buf[0]);
	memset(buf, 0, 2);
	lm75a_2bytes_read(fd, I2C_ADDR, LM75A_TOS_REG, buf);
	printf("TOS: %f C\n", OS_to_C(buf));
	memset(buf, 0, 2);
	lm75a_2bytes_read(fd, I2C_ADDR, LM75A_THYST_REG, buf);
	printf("Thyst: %f C\n", OS_to_C(buf));
	sleep(1);

	printf("Start monitor Temperature....\n");
	while (1) {
		usleep(80000);
		memset(buf, 0, 2);
