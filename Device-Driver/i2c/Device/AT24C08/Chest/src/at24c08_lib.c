 ...
 */
int at24c08_Random_read(int fd, unsigned char addr, 
 ...
 */
int at24c08_Sequen_read(int fd, unsigned char addr,
 ...
 *
 */
int at24c08_Current_Address_read(int fd, unsigned char addr, 
						unsigned char *buf)
{
	struct i2c_rdwr_ioctl_data msgset;
	struct i2c_msg msgs;
 ...
 *
 */
int at24c08_Byte_write(int fd, unsigned char addr, unsigned char offset, 
					unsigned char data)
{
	struct i2c_rdwr_ioctl_data msgset;
	struct i2c_msg msgs;
 ...
 *
 */
int at24c08_Page_write(int fd, unsigned char addr, unsigned char offset,
...
int at24c08_packet_read(int fd, unsigned char addr, unsigned char offset,
					unsigned char *buf, int len)
{
	int chunk, remain;
	int idx, ret;

	chunk = div_up(len, I2C_CHUNK_SIZE);
	remain = len % I2C_CHUNK_SIZE;

	/* Chunk read */
	for (idx = 0; idx < chunk; idx++) {
		ret = at24c08_Sequen_read(fd, addr, 
				offset + idx * I2C_CHUNK_SIZE,
				buf + idx * I2C_CHUNK_SIZE, I2C_CHUNK_SIZE);
		if (ret < 0)
			return ret;
	}
...
int at24c08_packet_write(int fd, unsigned char addr, unsigned char offset,
					unsigned char *buf, int len)
{
	int chunk, remain;
	int idx, ret;
	...
	/* Chunk write */
	for (idx = 0; idx < chunk; idx++) {
		ret = at24c08_Page_write(fd, addr, 
				offset + idx * I2C_CHUNK_SIZE, 
				buf + idx * I2C_CHUNK_SIZE, I2C_CHUNK_SIZE);
		if (ret < 0)
			return ret;
