...
int main()
{
	...
	buf[3] = 0x03;
	buf[4] = 0x04;
	ret = Page_write(fd, I2C_ADDR, 0x01, buf, 5);
	if (ret < 0) {
	...
	/* Current Address Read */
	ret = Current_Address_read(fd, I2C_ADDR, buf);
	if (ret < 0) {
		printf("I2C Current Address Read error.\n");
		close(fd);
		return -1;
	}
	printf("Current Address Read DATA: %#x\n", buf[0]);

	/* Sequential Read */
	memset(buf, 0, 8);
	ret = Sequen_read(fd, I2C_ADDR, 0x01, buf, 5);
	if (ret < 0) {
