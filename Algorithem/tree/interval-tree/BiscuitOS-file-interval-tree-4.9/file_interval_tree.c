...
BiscuitOS_vma_interval_tree_compute_subtree_last(struct vm_area_struct *node)
{
	...
	if (node->shared.rb.rb_left) {
		subtree_last = rb_entry(node->shared.rb.rb_left,
		struct vm_area_struct, shared.rb)->shared.rb_subtree_last;
		if (max < subtree_last)
			max = subtree_last;
	}
	if (node->shared.rb.rb_right) {
		subtree_last = rb_entry(node->shared.rb.rb_right,
		struct vm_area_struct, shared.rb)->shared.rb_subtree_last;
		if (max < subtree_last)
			max = subtree_last;
	}
...
void BiscuitOS_vma_interval_tree_insert(
		struct vm_area_struct *node, struct rb_root *root)
{
	...
	rb_insert_augmented(&node->shared.rb,
...
void BiscuitOS_vma_interval_tree_remove(
		struct vm_area_struct *node, struct rb_root *root)
{
	rb_erase_augmented(&node->shared.rb,
			root, &BiscuitOS_vma_interval_tree_augment);
}
...
BiscuitOS_vma_interval_tree_subtree_search(struct vm_area_struct *node,
...
BiscuitOS_vma_interval_tree_iter_first(struct rb_root *root,
				unsigned long start, unsigned long last)
{
	struct vm_area_struct *node;
