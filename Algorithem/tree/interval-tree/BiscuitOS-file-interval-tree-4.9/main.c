/*
 * Fork usage for file interval tree on BiscuitOS
 *
...
static int __init BiscuitOS_init(void)
{
	...
	struct vm_area_struct *fvma0, *fvma1;
	...
	/* Emulate fork Child Process */
	fvma0 = kzalloc(sizeof(*fvma0), GFP_KERNEL);
	if (!fvma0) {
		printk("Fork Child VMA create failed.\n");
		ret = -ENOMEM;
		goto err_fork0;
	}
	printk("Proccess VMA: %#lx\n", (unsigned long)fvma0);
	*fvma0 = *vma;
	...
	if (!fvma1) {
		printk("Fork Child Child VMA create failed.\n");
		ret = -ENOMEM;
		goto err_fork1;
	}
	printk("Proccess VMA: %#lx\n", (unsigned long)fvma1);
	*fvma1 = *fvma0;
	...
	/* remove */
	BiscuitOS_vma_interval_tree_remove(fvma1, &mapping->i_mmap);
	BiscuitOS_vma_interval_tree_remove(fvma0, &mapping->i_mmap);
