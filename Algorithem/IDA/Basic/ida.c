...
static int ida_pre_get(struct ida *ida, gfp_t gfp)
{
	/*
	...
	if (!ida_bitmap) {
		struct ida_bitmap *bitmap = (struct ida_bitmap *)malloc(
						sizeof(struct ida_bitmap));
...
err:
	printf("ida_free called for id=%d which is not allocated.\n", id);
}
